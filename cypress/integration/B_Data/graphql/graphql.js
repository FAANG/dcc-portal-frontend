export class GraphqlPage {
  ...
  check_tables_selection() {
    cy.get('mat-select[name=firstIndex]').click().get('mat-option').contains('analysis').click();
    cy.get('mat-select[name=secondIndex]').click().get('mat-option').contains('article').click();
  ...
  check_filters(firstIndex, secondIndex, queryTaskName, queryResultName, filterName, filterValue,
    ...
    cy.get('mat-select[name=firstIndex]').click().get('mat-option').contains(firstIndex).click();
    cy.get('mat-select[name=secondIndex]').click().get('mat-option').contains(secondIndex).click();
    cy.get('app-index-filters > :nth-child(1) > .mat-toolbar').should('exist')

    // select columns
    cy.get(`mat-select[name=${firstIndex}]`).click().get('mat-option').contains(option1).click();
    cy.get('div[role="listbox"]').should('exist')
    cy.get(`mat-select[name=${firstIndex}]`).get('mat-option').contains(option2).click();
    cy.get('body').click()
    cy.get('div[role="listbox"]').should('not.exist')

    // cy.get('mat-select[name=filterName0]').click()//.get('mat-option').contains('accession').click();
