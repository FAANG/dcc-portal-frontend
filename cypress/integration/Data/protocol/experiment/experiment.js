export class ExperimentPage {
  ...
  compare_filter_value(filterAccessor, filterAccessorType, queryParam) {
    const protocolCountAccessor = '[title="Protocol"] > .mat-card > :nth-child(2) > :nth-child(1) > .badge'
    cy.get(protocolCountAccessor)
      .invoke('text')
      .then((protocolCount) => {
        ...
        cy.get(protocolCountAccessor)
          .invoke('text')
          .should((updatedProtocolCount) => {
            expect(protocolCount).not.to.eq(updatedProtocolCount)
          })
      })
  }
  ...
  verify_pagination() {
    const protocolType = 'tbody > :nth-child(1) > .cdk-column-protocol_type > a'
    cy.get(protocolType)
      .invoke('text')
      .then((firstVal) => {
