export class AnalysisPage {
  ...
  compare_filter_value(filterAccessor, filterAccessorType, queryParam) {
    const organisationCountAccessor = '[title="Organisation"] > .mat-card > :nth-child(2) > :nth-child(1) > .badge'
    cy.get(organisationCountAccessor)
      .invoke('text')
      .then((initialCount) => {
  ...
  verify_pagination() {
    const protocolName = 'tbody > :nth-child(1) > .cdk-column-protocol_name > a'
    cy.get(protocolName)
      .invoke('text')
      .then((firstVal) => {
