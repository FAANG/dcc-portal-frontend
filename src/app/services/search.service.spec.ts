...
describe('SearchService', () => {
  ...
  it('should return 8499 result files when search for scrofa', inject([SearchService], (service: SearchService) => {
    service.searchFile('scrofa', false).subscribe(data => {
      expect(data['hits']['total']).toEqual(8499);
    });
  }));

  it('should return 1221 result files when search for scrofa with excluding legacy data', inject([SearchService],
    (service: SearchService) => {
    service.searchFile('scrofa', true).subscribe(data => {
      expect(data['hits']['total']).toEqual(1221);
    });
  }));
  ...
  it('should return 4710 result specimens when search for scrofa', inject([SearchService],
    (service: SearchService) => {
    service.searchSpecimen('scrofa', false).subscribe(data => {
      expect(data['hits']['total']).toEqual(4710);
    });
  }));
  ...
  it('should return 94 result datasets when search for scrofa', inject([SearchService], (service: SearchService) => {
    service.searchDataset('scrofa', false).subscribe(data => {
      expect(data['hits']['total']).toEqual(94);
    });
  }));
